                ...
                <a href="" class="navbar-brand p-0">
                    ...
                    <div class="collapse navbar-collapse" id="navbarCollapse">
                        <div class="navbar-nav ms-auto py-0">                        
                            <?php
                            include "db.php"; // connection ke database

                                // Ambil main menu (level 1)
                                $mainMenu = mysqli_query($conn, 
                                    "SELECT * FROM menu WHERE menu_level=1 AND userlevel LIKE '%1,%' ORDER BY menu_sort ASC");

                                while ($menu = mysqli_fetch_assoc($mainMenu)) {
                                    // Semak kalau menu ni ialah "Info Latihan"
                                    if ($menu['menu_arah'] == '-' && $menu['menu_tajuk'] == 'Info Latihan') {
                                        // Ambil submenu khas untuk Info Latihan (ikut menu_arah = 'info')
                                        $submenu = mysqli_query($conn, 
                                            "SELECT * FROM menu WHERE menu_level=2 AND menu_arah='info' ORDER BY menu_sort ASC");
                                        ...
                                        while ($sub = mysqli_fetch_assoc($submenu)) {
                                            $link = "info.php?id=" . $sub['ID'];   // ubah ke info.php
                                            echo '<a href="'.$link.'" class="dropdown-item">'.$sub['menu_tajuk'].'</a>';
                                        }                            

                                        echo '</div>';
                                        echo '</div>';
                                    }else if ($menu['menu_arah'] == '-' && $menu['menu_tajuk'] == 'Pustaka') {
                                        // Ambil submenu khas untuk Info Latihan (ikut menu_arah = 'info')
                                        $submenu = mysqli_query($conn, 
                                            "SELECT * FROM menu WHERE menu_level=2 AND menu_arah='pustaka' ORDER BY menu_sort ASC");
                                        ...
                                        while ($sub = mysqli_fetch_assoc($submenu)) {
                                            // direct link ke pustaka.php ikut ID submenu
                                            $link = "pustaka.php?idmenu=" . $sub['ID']; 
                                            echo '<a href="'.$link.'" class="dropdown-item">'.$sub['menu_tajuk'].'</a>';
                                        }

                                        echo '</div>';
                                        echo '</div>';
                                    }else if ($menu['menu_arah'] == '-' && $menu['menu_tajuk'] == 'Direktori') {
                                        // Ambil submenu khas untuk Direktori (ikut menu_arah = 'direktori')
                                        $submenu = mysqli_query($conn, 
                                            "SELECT * FROM menu WHERE menu_level=2 AND menu_arah='direktori' ORDER BY menu_sort ASC");
                                        ...
                                        while ($sub = mysqli_fetch_assoc($submenu)) {
                                            // Set link terus ke direktori.php dengan hantar ID submenu
                                            $link = "direktori.php?id=" . $sub['ID'];
                                            echo '<a href="'.$link.'" class="dropdown-item">'.$sub['menu_tajuk'].'</a>';
                                        }
                                        ...
                                        // Menu biasa (tak ada submenu)
                                        $link = ($menu['menu_url'] == '-' || empty($menu['menu_url'])) 
                                                ? "#" 
                                                : $menu['menu_url'];
            ...
            <div class="container-fluid bg-breadcrumb">
                <div class="container text-center py-5" style="max-width: 900px;">
                    ...
                    <ol class="breadcrumb d-flex justify-content-center mb-0 wow fadeInDown" data-wow-delay="0.3s">
                        <li class="breadcrumb-item"><a href="index.html">Laman Utama</a></li>
