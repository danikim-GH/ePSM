        ...
        <div class="content-header wow fadeInUp" data-wow-duration="1s" data-wow-delay="0s" style="visibility: visible; animation-delay: 0.2s; animation-name: fadeInUp;">
            ...
            <div class="header-stats">
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-info">
                        <span class="stat-number" id="suspendedCount">0</span>
                        <span class="stat-label">Suspended Users</span> {{-- aku nak fetch total suspend user dekat sini --}}
                    </div>
                </div>
            </div>
        </div>
            ...
            @if(count($pending) > 0)
                ...
                <div class="cards-grid">
                    @foreach ($pending as $user)
                        <div class="user-card wow fadeInUp" data-wow-duration="1.8s" data-wow-delay="0s" style=" visibility: visible; animation-delay: 0.2s; animation-name: fadeInUp;" data-user-id="{{ $user->NoKP }}">
                            <div class="card-header">
                                ...
                                <div class="user-status">
                                    ...
                                    @php
                                        [$text, $class, $icon] = $user->getStatusBadge();   
                                    @endphp
                            ...
                            <div class="user-info">
                                ...
                                <div class="info-group info-group-department">
                                    <label><i class="fas fa-building info-icon"></i> 
                                        Jabatan
                                    </label>
                                    <p class="info-value" id="jabatan-{{ $user->NoKP }}">
                                        {{ ucwords(strtolower($user->NamaJabatan ?? '-')) }} 
                                    </p>
                                </div>
                            </div>

                            <div class="user-actions">
                                <form id="approve-firm-{{ $user->NoKP }}" action="{{ route('admin.approve', $user->NoKP) }}" method="POST" class="action-form">
                                    @csrf
                                ...
                                <form action="{{ route('admin.suspendUser', $user->NoKP) }}" method="POST">
                                    @csrf
                                    <div class="action-buttons">
                                        <button class="btn btn-view btn-act-error" type="submit">
